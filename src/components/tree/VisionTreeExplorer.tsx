...
export default function VisionTreeExplorer() {
  ...
  const branches = root.childrenIds
    .map((id) => visionTreeData[id])
    .filter(Boolean);
  ...
  const handleBranchClick = (branchId: string) => {
    ...
    window.scrollTo({
      top:
        document.querySelector(".tree-explorer")?.getBoundingClientRect().top! +
        window.scrollY -
        20,
      behavior: "smooth",
    });
...
// Single manga-panel sentence — large, centered, animated on scroll
function MangaLine({
  text,
  variant = "normal",
  showPrefix,
  ...
  text: string;
  variant?: "normal" | "principle" | "analogy";
  showPrefix?: string;
}) {
  const ref = useRef<HTMLDivElement>(null);
  ...
  return (
    <motion.div
      ref={ref}
      className={`manga-line manga-line--${variant}`}
      initial={{ opacity: 0, y: 16 }}
      animate={isInView ? { opacity: 1, y: 0 } : {}}
      transition={{ duration: 0.5 }}
    >
      {showPrefix && <span className="manga-line__prefix">{showPrefix}</span>}
      <p className="manga-line__text">{text}</p>
    </motion.div>
  );
}

// ── Branch intro text ──
function WebtoonIntro({
  branch,
  color,
}: {
  branch: VisionNode;
  ...
  const mainLines = splitSentences(branch.content.mainText);
  const principleLines = branch.content.firstPrinciple
    ? splitSentences(branch.content.firstPrinciple)
    : [];
...
function WebtoonPanel({
  ...
  const principleLines = node.content.firstPrinciple
    ? splitSentences(node.content.firstPrinciple)
    : [];
  const analogyLines = node.content.analogy
    ? splitSentences(node.content.analogy)
    : [];
...
function QuoteBubble({
  ...
  return (
    <div className="wt-quote" style={{ borderColor: `${color}33` }}>
      ...
      {jpTranslation && <div className="wt-quote__jp">{jpTranslation}</div>}
      {source && (
    ...
    <div className="wt-data">
      {data.map((d, i) => (
        <div key={i} className="wt-data__card">
          <span className="wt-data__value" style={{ color }}>
            {d.value}
          </span>
